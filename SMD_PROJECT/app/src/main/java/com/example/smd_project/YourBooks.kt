...
class YourBooks : AppCompatActivity() {
        ...
        bookId.let { BOOKID ->
            databaseRef.child(BOOKID).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    // Parse the date from timee string
                    val dateFormat = SimpleDateFormat("dd-MM-yy", Locale.getDefault())
                    val borrowedDate = dateFormat.parse(timee)

                    // Calculate the difference in days
                    val currentDate = Calendar.getInstance().time
                    val difference = currentDate.time - borrowedDate.time
                    val daysPassed = TimeUnit.DAYS.convert(difference, TimeUnit.MILLISECONDS)

                    // Update the UI based on the difference
                    if (daysPassed < 7) {
                        val daysLeft = 7 - daysPassed
                        time.text = "Days left: $daysLeft"
                    } else {
                        time.text = "Days overbooked: ${daysPassed - 7}"
                        time.setTextColor(ContextCompat.getColor(this@YourBooks, android.R.color.holo_red_dark))
                        time.textSize = 20f
                    }

                    Glide.with(this@YourBooks)
